  ...
  for (const m of html.matchAll(re)) {
    try {
      ...
      for (const node of arr) {
        const offers = node?.offers ?? node?.offer ?? null;
        if (!offers) continue;
        const list = Array.isArray(offers) ? offers : [offers];
        for (const o of list) {
          const p = o?.price ?? o?.lowPrice ?? o?.highPrice;
          if (p && String(p).match(/^\d[\d.,]*$/)) return { ok: true, evidence: snippet(html, m.index ?? 0) };
        }
      }
    } catch {}
  }
...
function looseCurrency(html: string) {
  const m = /(₹|\$|€|£)\s*\d[\d.,]{1,}/.exec(html);
  if (m) return { ok: true as const, evidence: snippet(html, m.index) };
  return { ok: false as const };
}
function hasPrice(html: string) {
  return fromJsonLd(html).ok ? fromJsonLd(html)
    : fromMeta(html).ok ? fromMeta(html)
    : fromMicrodata(html).ok ? fromMicrodata(html)
    : looseCurrency(html);
}
...
export type SimResult = {
  url: string; ok: boolean; failures: string[]; millis: number; evidence?: string | null;
};
  ...
  for (const url of urls) {
    ...
    try {
      ...
      if (assertions.textIncludes) {
        ...
        const i = html.toLowerCase().indexOf(q.toLowerCase());
        if (i === -1) fails.push(`MISSING:Text("${q}")`); else evidence = evidence ?? snippet(html, i);
      }
      if (assertions.price) {
        const pr = hasPrice(html); if (!pr.ok) fails.push("MISSING:Price"); else evidence = evidence ?? pr.evidence ?? null;
      }
      if (assertions.atc) {
        const atc = hasATC(html); if (!atc.ok) fails.push("MISSING:AddToCart"); else evidence = evidence ?? atc.evidence ?? null;
      }

      out.push({ url, ok: fails.length === 0, failures: fails, millis: Date.now() - start, evidence });
    } catch (e: any) {
      out.push({ url, ok: false, failures: [`FETCH_ERROR:${e?.name || "Error"}`], millis: Date.now() - start, evidence: null });
    }
  }
  clearTimeout(t);
  return out;
